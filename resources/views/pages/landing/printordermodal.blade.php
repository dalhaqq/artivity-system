<button data-modal-target="printOrderAdmin" data-modal-toggle="printOrderAdmin" type="button" class="hidden"></button>
<div wire:ignore.self id="printOrderAdmin" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full transition-all">
    <div class="relative w-full max-w-2xl max-h-full transition-all">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 transition-all">
            ...
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600 transition-all">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="printOrderAdmin">
            ...
            <div class="flex justify-between items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                <div class="space-y-2 text-left">
                    <button wire:click='modalPayment' type="button" class="rounded-md bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-600 shadow-sm hover:bg-indigo-100">Bukti Pembayaran</button>
                ...
                @if ($Order_status_id == 'Paid')         
                <div class="flex flex-none items-center gap-x-2">
                    <button wire:click='modalAcceptOrder' type="button"
                    class="rounded-md whitespace-nowrap px-2.5 py-1.5 font-semibold ring-1 ring-inset text-green-700 bg-green-50 ring-green-600/20">
                    Terima
                    </button>
                    <button wire:click='modalCancelOrder' type="button"
                        class="rounded-md whitespace-nowrap px-2.5 py-1.5 font-semibold ring-1 ring-inset text-red-700 bg-red-50 ring-red-600/20">
                        Tolak
                    </button>
                </div>
                @endif
