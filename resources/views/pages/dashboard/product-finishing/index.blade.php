<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="relative bg-indigo-200 p-4 sm:p-6 rounded-xl overflow-hidden mb-8">
            ...
            <!-- Content -->
            <div class="relative">
                <h1 class="text-2xl md:text-3xl text-slate-800 font-bold mb-1">Finishing Produk</h1>
                <nav class="flex" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <div class="flex items-center">
                            <svg aria-hidden="true" class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Finishing Produk</span>
                        </div>
                        </li>
                    </ol>
                </nav>
        ...
        <div class="grid grid-cols-12 gap-6">
            ...
            <div class="h-fit col-span-full xl:col-span-6 bg-white shadow-lg rounded-xl border border-gray-200">
                <header class="px-5 py-4 border-b border-gray-100">
                    <h2 class="font-semibold text-gray-800 judul"><i class="fa-solid fa-plus"></i> Tambah Finishing Produk</h2>
                </header>
                <div class="px-5 py-3">
                    <div class="flex items-start">
                        <div class="judulform text-3xl font-bold text-gray-800 mb-3">Input Finishing Produk</div>
                    </div>
                    ...
                    <div id="alert-3" class="flex p-4 mb-4 text-green-800 rounded-lg bg-green-100 dark:bg-gray-800 dark:text-green-400" role="alert">
                        ...
                        <div class="ml-3 text-sm font-medium">
                            berhasil <span class="font-bold">{{ session('berhasil') }}</span> data finishing produk.
                        </div>
                    ...
                    @endif
                    <form class="forminputdata" action="{{ route('finishing-product.store') }}" method="post">
                        @csrf
                        <div class="flex flex-col mb-3">
                            <x-jet-label for="name" value="{{ __('Nama') }}" />
                            <x-jet-input placeholder="Apa finishing yang ingin anda tambahkan?" id="name" type="text" name="name" :value="old('name')" required autofocus />
                        </div>
                        <div class="flex flex-col mb-3">
            ...
            <div class="col-span-full xl:col-span-6 bg-white shadow-lg rounded-xl border border-gray-200">
                <header class="px-5 py-4 border-b border-gray-100">
                    <h2 class="font-semibold text-gray-800">Data Finishing Produk</h2>
                </header>
                <div class="p-3">
                        ...
                        <table id="myTable" class="table-auto w-full">
                            ...
                            <thead class="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
                                <tr>
                                    ...
                                    <th class="p-2 whitespace-nowrap">
                                        <div class="font-semibold text-left cursor-pointer">Nama</div>
                                    </th>
                                    <th class="p-2 whitespace-nowrap">
                                        <div class="font-semibold text-left cursor-pointer">Deskripsi</div>
                                    </th>
                                    <th class="p-2 whitespace-nowrap">
                                        <div class="font-semibold text-left cursor-pointer">Harga</div>
                                    </th>
                                    <th class="p-2 whitespace-nowrap">
                            ...
                            <tbody class="text-sm divide-y divide-gray-100">
                                ...
                                @foreach ($data as $finishing)
                                    <tr>
                                        ...
                                        <td class="p-2 whitespace-nowrap">
                                            <div class="text-left font-medium">{{ $finishing->name }}</div>
                                        </td>
                                        <td class="p-2 whitespace-nowrap">
                                            <div class="text-left font-medium">{{ $finishing->description }}</div>
                                        </td>
                                        <td class="p-2 whitespace-nowrap">
                                            <div class="text-left font-medium">{{ $finishing->price }}</div>
                                        </td>

                                        <td class="p-2 whitespace-nowrap flex justify-center gap-2">
                                            <button data-idfinishing="{{ $finishing->id }}" class="editfinishing text-gray-800 btn bg-yellow-300 hover:bg-yellow-400"><i class="fa-solid fa-pen-to-square"></i></button>
                                            <button data-idfinishing="{{ $finishing->id }}"data-modal-target="popup-modal" data-modal-toggle="popup-modal" type="button"class="hapusfinishing text-gray-800 btn bg-red-300 hover:bg-red-400"><i class="fa-solid fa-trash"></i></button>
                                        </td>
                                    </tr>
                                <?php $i++?>
                                @endforeach
        ...
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500">Apakah anda yakin untuk menghapus?</h3>
    ...
    <script>
        $(function(){
            $('.hapusfinishing').on('click', function(){
                    const id = $(this).data('idfinishing');
                    $('form.formhapus').attr('action', '{{ env('APP.URL') }}'+'finishing-product/' +id )
                });
        });

        $(function(){
            $('.editfinishing').on('click', function(){
                const id = $(this).data('idfinishing');
                $('.judulform').html('Edit Finishing Produk')
                $('.judul').html('Edit Finishing Produk')
                ...
                $('.forminputdata').attr('action', '{{ env('APP.URL') }}'+'finishing-product/' +id)
                $('.forminputdata').prepend('{{ method_field('PUT') }}')

                $.ajax({
                    url: '{{ env('APP.URL') }}'+'jsonfinishing/'+id,
                    method: 'get',
                    dataType: 'json',
